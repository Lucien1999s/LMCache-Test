...
"""Summarize ContextFlow accuracy JSONL results into CSV."""
...
def _metric_summary(values: Iterable[Any]) -> dict[str, float | None]:
    numeric = [value for raw in values if (value := _as_float(raw)) is not None]
    return {
...
def _bool_mean(rows: list[dict[str, Any]], field: str) -> float | None:
...
def summarize(input_path: Path) -> list[dict[str, Any]]:
    """Summarize per-example JSONL rows."""
    rows = [
        json.loads(line)
    ...
    for key, group_rows in sorted(grouped.items(), key=lambda item: item[0]):
        ...
        out["selected_answer_recall"] = _bool_mean(
            group_rows,
            "selected_contains_answer",
        )
        out["full_context_answer_recall"] = _bool_mean(
            group_rows,
...
def main() -> None:
    ...
    for metric in METRIC_FIELDS:
        for stat_name in ["mean", "median", "p95", "max", "min"]:
