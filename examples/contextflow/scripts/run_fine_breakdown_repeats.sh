...
CONDITIONS="${CONDITIONS:-16:1:20 30:16:20}"

OPTIONAL_FULL_SWEEP="${OPTIONAL_FULL_SWEEP:-0}"
FULL_SWEEP_CHUNK_COUNTS="${FULL_SWEEP_CHUNK_COUNTS:-2 4 8 16 24 30}"
FULL_SWEEP_MAX_NEW_TOKENS_LIST="${FULL_SWEEP_MAX_NEW_TOKENS_LIST:-1 16}"
FULL_SWEEP_REPEATS="${FULL_SWEEP_REPEATS:-5}"

RESULT_DIR="${RESULT_DIR:-examples/contextflow/results}"
OUT="${OUT:-$RESULT_DIR/contextflow_finer_breakdown_repeats_runs.jsonl}"
PROFILE_OUT="${PROFILE_OUT:-$RESULT_DIR/contextflow_finer_breakdown_repeats_events.jsonl}"
EXCLUSIVE_EVENTS_OUT="${EXCLUSIVE_EVENTS_OUT:-$RESULT_DIR/contextflow_finer_breakdown_repeats_exclusive_events.jsonl}"
EXCLUSIVE_SUMMARY_OUT="${EXCLUSIVE_SUMMARY_OUT:-$RESULT_DIR/contextflow_finer_breakdown_repeats_exclusive_summary.csv}"
STATS_OUT="${STATS_OUT:-$RESULT_DIR/contextflow_finer_breakdown_repeats_stats.csv}"

mkdir -p "$RESULT_DIR" /tmp/cf

if [[ "$OPTIONAL_FULL_SWEEP" == "1" ]]; then
  for c in $FULL_SWEEP_CHUNK_COUNTS; do
    for max_new_tokens in $FULL_SWEEP_MAX_NEW_TOKENS_LIST; do
      CONDITIONS="$CONDITIONS $c:$max_new_tokens:$FULL_SWEEP_REPEATS"
    done
  done
fi
...
echo "Exclusive events: $EXCLUSIVE_EVENTS_OUT"
echo "Exclusive summary: $EXCLUSIVE_SUMMARY_OUT"
echo "Repeat stats: $STATS_OUT"

run_one () {
  local chunk_count="$1"
  local max_new_tokens="$2"
  local repeat_id="$3"
  local run_id="lmcache_blend-fine-repeat-c${chunk_count}-tok${max_new_tokens}-r${repeat_id}-$(date +%s%N)"
...
"$PYTHON_BIN" examples/contextflow/src/summarize/fine_exclusive.py \
  "$PROFILE_OUT" \
  --summary-output "$EXCLUSIVE_SUMMARY_OUT" \
  --events-output "$EXCLUSIVE_EVENTS_OUT"

"$PYTHON_BIN" examples/contextflow/src/summarize/fine_repeat_stats.py \
...
echo "Fine-event JSONL:      $PROFILE_OUT"
echo "Exclusive events JSONL:$EXCLUSIVE_EVENTS_OUT"
echo "Exclusive summary CSV: $EXCLUSIVE_SUMMARY_OUT"
echo "Repeat stats CSV:      $STATS_OUT"
